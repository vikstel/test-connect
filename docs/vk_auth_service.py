"""
VK ID OAuth 2.1 + PKCE — Сервис авторизации для BlogToSocial (Django)
...
Настройки в Django settings.py:
  VK_APP_ID = "your_app_id"              # ID приложения из id.vk.com
  VK_APP_SECRET = "your_app_secret"      # Защищённый ключ приложения
...
def get_authorization_url(state: str | None = None) -> dict:
    ...
    Args:
        state: Произвольная строка для защиты от CSRF.
               Если не передана — генерируется автоматически.

    Returns:
        {
            "url": "https://id.vk.com/authorize?...",
            "state": "...",
            "code_verifier": "..."  # СОХРАНИТЬ В СЕССИЮ!
        }
    """
    if state is None:
    ...
    params = {
        ...
        "scope": getattr(settings, "VK_OAUTH_SCOPES", "wall,photos,groups,offline"),
        "state": state,
    ...
    return {
        "url": url,
        "state": state,
        "code_verifier": code_verifier,  # Сохранить в сессию пользователя!
    }
...
def exchange_code_for_token(code: str, code_verifier: str, device_id: str = "") -> dict:
    ...
    Args:
        ...
        code_verifier: Сохранённый ранее code_verifier из сессии.
        device_id: device_id из callback URL (VK ID передаёт его).
...
def call_vk_api(method: str, access_token: str, **params) -> dict:
    ...
    params["v"] = getattr(settings, "VK_API_VERSION", "5.199")
