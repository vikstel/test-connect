"""
Обновлённый VKPoster с интеграцией VK ID OAuth 2.1

Изменения по сравнению с оригинальной версией:
1. Автоматический refresh токена при ошибке авторизации
2. Логирование каждого шага загрузки фото
3. Ошибка загрузки фото НЕ проглатывается — пост не публикуется без медиа
4. Поддержка нескольких фото в одном посте
"""
...
from .vk_auth_service import call_vk_api, refresh_access_token
...
class VKPoster:
    ...
    Args:
        ...
        on_token_refreshed: Callback для сохранения нового токена в БД (опционально)
    """
    ...
    def _try_refresh_token(self) -> bool:
        ...
        if result["ok"]:
            ...
            # Уведомляем о новом токене для сохранения в БД
    ...
    def _upload_photo(self, media_path: str) -> str:
        ...
        logger.info(f"[VK Upload] Step 1 OK: got upload_url")
        ...
        logger.info(f"[VK Upload] Step 3: photos.saveWallPhoto")
